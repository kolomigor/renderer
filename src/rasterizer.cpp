...
float interpolateDepth(const Triangle& triangle, float alpha, float beta, float gamma) {
	const float reciprocal_w = alpha * triangle.v0.position.w + beta * triangle.v1.position.w +
	                           gamma * triangle.v2.position.w;
	const float depth_over_w = alpha * triangle.v0.position.z * triangle.v0.position.w +
	                           beta * triangle.v1.position.z * triangle.v1.position.w +
	                           gamma * triangle.v2.position.z * triangle.v2.position.w;
	return depth_over_w / reciprocal_w;
}
